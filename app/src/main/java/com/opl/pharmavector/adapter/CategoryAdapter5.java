...
public class CategoryAdapter5 extends ArrayAdapter<Category4> {
    private final List<Category4> list;

    public CategoryAdapter5(Context context, int resource, List<Category4> list) {
        ...
        if (convertView == null) {
            ...
            viewHolder.categoryCheckBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
                ...
                public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                    ...
                    Log.e("getPositionvalues", FavouriteCategoriesJsonParser5.selectedCategories5.toString());

                    list.get(getPosition).setSelected(buttonView.isChecked());
                    if (buttonView.isChecked()) {

                        if (!FavouriteCategoriesJsonParser5.selectedCategories5.contains(String.valueOf(list.get(getPosition).getCateogry_id()))) {
                            FavouriteCategoriesJsonParser5.selectedCategories5.add(String.valueOf(list.get(getPosition).getCateogry_id()));

                        }
                    } else {

                        if (FavouriteCategoriesJsonParser5.selectedCategories5.contains(String.valueOf(list.get(getPosition).getCateogry_id()))) {
                            FavouriteCategoriesJsonParser5.selectedCategories5.remove(String.valueOf(list.get(getPosition).getCateogry_id()));
