...
public class Offlineordupdate extends Activity implements OnClickListener {
    ...
    OfflineProductListupdateAdapter adapter2;
    Button submit;
    ...
    int textlength = 0;
    OfflineProductListupdateAdapter  adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(layout.offlineordupdate);
        ...
        OfflineProductListupdateAdapter .qnty = "10";
        OfflineProductListupdateAdapter .qntyID.clear();
        OfflineProductListupdateAdapter .qntyVal.clear();
    ...
    protected void onResume() {
        // TODO Auto-generated method stub
        // System.out.println("EditTxtID " +   OfflineProductListupdateAdapter .editTxtID.size());
        super.onResume();
    }
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == submit.getId()) {
            Toast.makeText(getBaseContext(), "Clicked to Submit Order", Toast.LENGTH_LONG).show();
            ...
            String last_quantity =   OfflineProductListupdateAdapter.qnty;
            int last_position =   OfflineProductListupdateAdapter.last_position;
            if (last_quantity != null) {
                OfflineProductListupdateAdapter.dataSet.put(last_position, last_quantity);
                OfflineProductListupdateAdapter.qntyID.add(last_position);
                OfflineProductListupdateAdapter.qntyVal.add(last_quantity);
                OfflineProductListupdateAdapter.set2.add(last_position);

            }
            if (  OfflineProductListupdateAdapter.qntyID.size() < 1) {
                Toast.makeText(this, "No item inserted", Toast.LENGTH_SHORT).show();
            }
            else {
                String qnty;
                ArrayList<Integer> position = new ArrayList<Integer>();
                for (int j :   OfflineProductListupdateAdapter.set2) {
                    position.add(j);
                }
                int k=0;

                int submit_value=0;
                for (int i = 1; i <OfflineProductListupdateAdapter.p_quanty.size(); i++) {
                    int value = Integer.parseInt(OfflineProductListupdateAdapter.p_quanty.get(i));
                    if (value > 0) {
                        submit_value=submit_value+value;
                        k=k+1;
                        int qnty1 = Integer.parseInt(OfflineProductListupdateAdapter.p_quanty.get(i));
                        params.add(new BasicNameValuePair("total", String.valueOf(k)));
                        params.add(new BasicNameValuePair("id"+ String.valueOf(k),p_code.get(i - 1)));
                        params.add(new BasicNameValuePair("ORD_QNTY"+ String.valueOf(k),OfflineProductListupdateAdapter.p_quanty.get(i)));
                        ...
                        String ORD_QNTY= OfflineProductListupdateAdapter.p_quanty.get(i);
                ...
                params.add(new BasicNameValuePair("SUBMIT_TYPE","Offline"));
                        ...
                        public void run() {
                            ...
                            try {
                                ...
                                if (success_1 == 1) {
                                    progress.dismiss();
                                    // startActivity(i);
                                    OfflineProductListupdateAdapter.qnty = null;
                                    OfflineProductListupdateAdapter.qntyID.clear();
                                    OfflineProductListupdateAdapter.qntyVal.clear();
                                    OfflineProductListupdateAdapter.set2.clear();
                                    OfflineProductListupdateAdapter.dataSet.clear();
                                    OfflineProductListupdateAdapter.p_quanty.clear();
                                    OfflineProductListupdateAdapter.mProductSerialList.clear();
                                } else {
                            ...
                            Intent sameint = new Intent(Offlineordupdate.this, ReadComments.class);
        ...
        if (v.getId() == calc.getId()) {
            ...
            int total_item =   OfflineProductListupdateAdapter .p_quanty.size();
            // System.out.println("total_item " + total_item);
            for (int i = 1; i < total_item; i++) {
                int value = Integer
                        .parseInt(  OfflineProductListupdateAdapter .p_quanty.get(i));
                // System.out.println("value " + value);

                if (value > 0) {

                    qnty1 = Integer.parseInt(  OfflineProductListupdateAdapter .p_quanty.get(i));
