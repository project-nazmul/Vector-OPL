...
public class ProductListupdateAdapter extends BaseAdapter implements Filterable {
	...
	public ProductListupdateAdapter(Activity activity,
	...
	ProductListupdateAdapter(Context con, ArrayList<String> sl,
							 ArrayList<String> p_name, HashMap<Integer, String> p_quanty) {

		this.p_names = p_name;
		this.mStringList = p_name;
		ProductListupdateAdapter.p_quanty = p_quanty;
		this.sl = sl;
		this.mContext = con;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		int srl = ProductupdateOrder.nameSerialPair.get(holder.p_name.getText().toString());
		...
		holder.quantity.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				//ProductupdateOrder.searchview.setFocusable(false);
				...
				try {
					...
					if (str.toString().isEmpty()) {
						...
						for (Iterator i = keys.iterator(); i.hasNext();) {
							...
							String product_rate = ProductupdateOrder.categoriesList.get(key).getPROD_RATE();
						...
						ProductupdateOrder.totalsellquantity.setText("" + tota);
						return;
					}
					else{
						...
						if (checkZero > 0) {
							...
							for (Iterator i = keys.iterator(); i.hasNext();) {
								...
								String product_rate = ProductupdateOrder.categoriesList.get(8).getPROD_RATE();
		...
		holder.quantity.setOnFocusChangeListener(new View.OnFocusChangeListener() {
			...
			public void onFocusChange(View v, boolean hasFocus) {
				...
				if (!hasFocus) {
					...
					//ProductupdateOrder.searchview.setFocusableInTouchMode(false);
			...
			try {
				...
				if (myNum> 0) {
					...
					for (int i = 0; i < p_names.size(); i++) {
						...
						//System.out.println("text Textttttttttttt\t" + productName);
						// System.out.println("P_Serial "+Integer.parseInt(sl.get(i)));
						myNum=1;
